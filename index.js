  ...
  `}const Ee=async e=>{const s=e.target.closest("button");if(!s)return;const t=Math.ceil(k/w);s.dataset.action==="prev"&&c>1&&(c-=1),s.dataset.action==="next"&&c<t&&(c+=1),s.dataset.page&&(c=+s.dataset.page);const n=o.petsList.getBoundingClientRect().top+window.pageYOffset-200;window.scrollTo({top:n,behavior:"smooth"}),V(),await P(c,C),N()};o.petsPagination.addEventListener("click",Ee);const $e=async()=>await W("https://paw-hut.b.goit.study/api/categories"),Be=e=>{e.unshift({name:"Всі"});const s=["Всі","Собаки","Коти","Кролики","Гризуни","Птахи","Тварини з особливими потребами","Терміново шукають дім"];return e.sort((t,n)=>s.indexOf(t.name)-s.indexOf(n.name)),e.map(t=>`<li class="pets-ctgs-item"><button class="pets-ctgs-btn" aria-pressed="false" data-id="${t._id}" type="button">${t.name}</button></li>`).join("")},Ce=async()=>{try{const e=await $e();o.ctgsList.innerHTML=Be(e.data);const s=document.querySelector(".pets-ctgs-btn");s.classList.add("active"),s.dataset.id="all",s.setAttribute("aria-pressed","true")}catch(e){ne(e.message)}},qe=async(e,s)=>{const t={page:e,limit:w};return s&&(t.categoryId=s),await W("https://paw-hut.b.goit.study/api/animals",{params:t})},K=e=>e.map(s=>`<li class="pets-item" role="listitem" data-id="${s._id}" data-description="${s.description||""}" data-health="${s.healthStatus||""}" data-behavior="${s.behavior||""}">
...
</li>`).join(""),P=async(e,s)=>{try{Xe();const t=await qe(e,s);Ae(),x()?o.petsList.insertAdjacentHTML("beforeend",K(t.data.animals)):o.petsList.innerHTML=K(t.data.animals),k=t.data.totalItems,o.petsList.querySelectorAll(".pets-item").forEach((i,l)=>{const u=t.data.animals[l];u&&(i.dataset.description=u.description||"",i.dataset.health=u.healthStatus||"",i.dataset.behavior=u.behavior||"")})}catch(t){ne(t.message)}},re=()=>o.petsList.innerHTML="",xe=e=>{document.querySelectorAll(".pets-ctgs-btn").forEach(t=>{t.classList.remove("active"),t.setAttribute("aria-pressed","false")}),e.classList.add("active"),e.setAttribute("aria-pressed","true")},Me=async e=>{e.target.nodeName==="BUTTON"&&(H(),V(),xe(e.target),C=e.target.dataset.id==="all"?void 0:e.target.dataset.id,c=1,re(),await P(c,C),x()&&k>w?_():x()||N())};o.ctgsList.addEventListener("click",Me);const Pe=async()=>{c++,await P(c,C),c*w>=k&&H()};o.petsLoadBtn.addEventListener("click",Pe);const H=()=>{o.petsLoadBtn.style.display="none"},_=()=>{o.petsLoadBtn.style.display="block"},Ae=()=>{o.petsLoader.style.display="none"},Xe=()=>{o.petsLoader.style.display="block"},V=()=>{o.petsPagination.style.display="none"},Ie=()=>{const e=oe();e!==q&&(re(),H(),V(),q=e,w=ie(),c=1,P(c,C),e==="mobile"&&c*w<k?_():e!=="mobile"&&N())};window.addEventListener("resize",Ie);const Oe=async()=>{await Ce(),await P(c),x()&&k>w?_():x()||N()};Oe();function Te(){return`
    ...
    <div class="details-modal-backdrop" data-details-modal-backdrop>
      <div class="details-modal" role="dialog" aria-modal="true" aria-labelledby="details-modal-title" aria-describedby="details-modal-description">
        ...
        </li>`).join("");o.successList.innerHTML=s},Ue=o.successAnimation,J=15;for(let e=0;e<J;e++){const s=document.createElement("div");s.classList.add("paw"),s.style.animationDelay=`${(J-e)*.25}s`;const t=document.createElementNS("http://www.w3.org/2000/svg","svg");t.classList.add("icon");const n=document.createElementNS("http://www.w3.org/2000/svg","use");n.setAttributeNS("http://www.w3.org/1999/xlink","xlink:href","#paw"),t.appendChild(n),s.appendChild(t),Ue.appendChild(s)}
